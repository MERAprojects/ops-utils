...
***************************************************************************/
void *vrfThread(void *arg)
{
   struct nlutils_op_data *tdata=(struct nlutils_op_data *)arg;

   if (vrf_setns_with_name(tdata->ns_name) != 0)
   {
	   _exit(EXIT_SUCCESS);
   }
...
int vrf_create_socket_using_id (const struct ovsdb_idl *idl, int64_t table_id,
                                struct vrf_sock_params *params)
{
    ...
    vrf_row = vrf_lookup_on_table_id(idl, table_id);
    if (vrf_row != NULL)
        return vrf_create_socket(vrf_row->name, params);
 ...
 ***************************************************************************/
int vrf_close_socket_using_id (const struct ovsdb_idl *idl, int64_t table_id,
                               int socket_fd)
{
    const struct ovsrec_vrf *vrf_row = NULL;
...
{
    return nl_setns_with_name(vrf_name);
}
 ...
 * @return 0 if sucessful, else negative value on failure
 ***************************************************************************/
int vrf_setns_with_id (const struct ovsdb_idl *idl, uint table_id)
{
     int fd = -1;
     char ns_path[MAX_BUFFER_SIZE] = {0};
     char vrf_ns_name[UUID_LEN] = {0};
     ...
     {
         VLOG_ERR("Unable to find namespace for table_id %d", table_id);
         return -1;
     }
...
* @param[in]  vrf_ns_name : this is the namespace in which search to be performed.
*
* @return valid ifindex if sucessful, else 0 on failure
***************************************************************************/
unsigned int vrf_if_nametoindex(const char *vrf_ns_name, const char* if_name)
{
...
***************************************************************************/
unsigned int
vrf_if_indextoname (const int ifindex, char *if_name, const char *vrf_ns_name)
{
    pthread_t tid;
    struct nlutils_op_data tdata;
...
vrf_is_ready (const struct ovsdb_idl *idl, char *vrf_name)
{
    ...
    if (vrf &&
        !strcmp(smap_get(&vrf->status, VRF_STATUS_KEY), VRF_STATUS_VALUE))
    {
        return true;
    }
    return false;
